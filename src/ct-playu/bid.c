...
void bid_phase(State *s, Strat *strat, long strat_cnt)
{
    ...
    sprintf(log_buff, "Stage:Bid, winner:%s, winning_bid:%u, bid_forced:%u, bid_stolen:%u\n", s->winning_bidder==0?"Me":"You", s->winning_bid>0?s->winning_bid+1:0, s->bid_forced, s->bid_stolen); log_msg(log_buff);
}
